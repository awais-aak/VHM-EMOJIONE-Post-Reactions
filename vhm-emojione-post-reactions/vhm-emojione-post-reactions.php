...
if(!class_exists('VHM_Emojione_Post_Reactions'))
{
    class VHM_Emojione_Post_Reactions
    {
        ...
        public function __construct()
        {
			...
			$this->plugin_url = admin_url('options-general.php?page=vhm_emojione_post_reactions');
		...
		{
			add_options_page(
				'VHM EMOJIONE Post Reactions', 
				'VHM EMOJIONE Post Reactions', 
				'manage_options',
				'vhm_emojione_post_reactions', 
		...
		public function admin_enqueue_scripts() {
			wp_enqueue_script( 'vhm_emojione_post_reactions_admin_js', plugins_url('/js/admin_scripts.js', __FILE__), false, false, true );
		}
		...
		public function frontend_enqueue_scripts() {
			...
			wp_enqueue_style('vhm_emojione_post_reactions_css', '//cdn.jsdelivr.net/emojione/2.1.3/assets/css/emojione.min.css');
			wp_enqueue_script( 'vhm_emojione_post_reactions_js', '//cdn.jsdelivr.net/emojione/2.1.3/lib/js/emojione.min.js', false, false, true );
			wp_enqueue_script( 'vhm_emojione_post_reactions_frontend_js', plugins_url('/js/frontend_scripts.js', __FILE__), false, false, true );
			wp_localize_script( 'vhm_emojione_post_reactions_frontend_js', 'vhm_emojione_var', array(
				'site_url' => site_url(),
			...
			echo "
				<style>
					#vhm-emojione-post-reactions-box { position: relative; }
					#vhm-emojione-post-reactions-box ol { width:100%; }
					#vhm-emojione-post-reactions-box ol li { width: " . $this->reaction_box_width . "%;display:inline-block;vertical-align:middle; }
					#vhm-emojione-post-reactions-box ol li.vote { cursor:pointer; }
					#vhm-emojione-post-reactions-box ol li .vhm-emojione-post-reaction-cell { text-align: center; }
					#vhm-emojione-post-reactions-box ol li span { }
					@media (max-width: 768px) {
					  #vhm-emojione-post-reactions-box ol li { width:50%;clear:both; }
					}
				</style>
			";
		}
		...
		public function vote()
		{
			...
			$get_voters = (array) get_post_meta($post_id, '_vhm_emojione_post_reactions_voters', true);
			if (!in_array($this->get_user_ip(), $get_voters))	
			{
				if (isset($this->options['reactions'][$item_voted]))
				{
					...
					$get_post_reactions = get_post_meta($post_id, '_vhm_emojione_post_reactions', true);
					$get_post_reactions[$item_voted] += 1;
					update_post_meta($post_id, '_vhm_emojione_post_reactions', $get_post_reactions);
					
					// Update voters list
					$get_voters[] = $this->get_user_ip();
					update_post_meta($post_id, '_vhm_emojione_post_reactions_voters', $get_voters);
				}
			}
			else
			{
		...
		public function load_reactions_box()
		{
			...
			if ($this->count_reactions > 0) {
				...
				$get_voters = get_post_meta($post_id, '_vhm_emojione_post_reactions_voters', true);
				
				$class = '';
				if (!in_array($this->get_user_ip(), (array)$get_voters))
					$class = 'vote';
				
				$return = '<div class="vhm-emojione-post-reactions-title">' . $this->options['title'] . '</div>';
				$return .= '<ol>';
				...
				foreach ($this->options['reactions'] as $k => $reaction) {
					...
					$return .= '<li class="' . $class . '" data-vhm_emojione_vote_id="' . $k . '"><div class="vhm-emojione-post-reaction-cell">';
					$return .= '<i class="vhm-emojione-post-reactions-code">' . $reaction['code'] . '</i><p><strong>' . $reaction['label'] . '</strong><br>' . $get_post_reaction . ' (' . $percent . '%)</p>';
					$return .= '</div></li>';
				} 
				$return .= '</ol>';
			}
...
add_shortcode( 'vhm_emojione_post_reactions', 'vhm_emojione_post_reactions' );
